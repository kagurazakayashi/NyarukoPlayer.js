...
": "+c.statusText),nyarukoplayer_animationinit(a))})}catch(e){nyarukoplayer_consolelog&&console.error("Data loading failed: "+e),nyarukoplayer_error()}}
...
function nyarukoplayer_play(a){if(void 0===a||!a)if(0==nyarukoplayer_retaincount)nyarukoplayer_retaincount++;else return;$("#nyarukoplayer").append('<div class="nyarukodiv"></div>');var c=$(".nyarukodiv"),e=nyarukoplayer_width[nyarukoplayer_now],b=nyarukoplayer_height[nyarukoplayer_now];c.append(nyarukoplayer_imgcache[nyarukoplayer_now]);var g=nyarukoplayer_to[nyarukoplayer_now];a=1E3*nyarukoplayer_time[nyarukoplayer_now];var d=nyarukoplayer_frame(nyarukoplayer_from[nyarukoplayer_now],e,b);e=nyarukoplayer_frame(g,
e,b);c.css({left:d[0],top:d[1],width:d[2],height:d[3]});c.animate({left:e[0],top:e[1],width:e[2],height:e[3]},a,function(){nyarukoplayer_consolelog&&console.log("[NyarukoPlayer] IMG "+(nyarukoplayer_now+1)+"/"+nyarukoplayer_count);nyarukoplayer_now<nyarukoplayer_count-1?(nyarukoplayer_now++,c.remove(),nyarukoplayer_play(!0)):(nyarukoplayer_consolelog&&console.log("[NyarukoPlayer] Animate End."),$.isFunction(nyarukoplayerCallback_AnimateEnd)&&nyarukoplayerCallback_AnimateEnd(),nyarukoplayer_replay&&
(nyarukoplayer_now=0,c.remove(),nyarukoplayer_play(!0)))})}
function nyarukoplayer_frame(a,c,e){var b=$("#nyarukoplayer").width(),g=$("#nyarukoplayer").height(),d=0,m=0,f=0,k=0,l=c/e;"L"==a?(d=0,f=l*g,f<b&&(f=b),k=f/l,m=(g-k)/2):"R"==a?(f=l*g,f<b&&(f=b),d=b-f,k=f/l,m=(g-k)/2):"U"==a?(m=0,k=b/l,k<g&&(k=g),f=k*l,d=(b-f)/2):"D"==a?(k=b/l,k<g&&(k=g),f=k*l,m=g-k,d=(b-f)/2):"C"==a?(a=nyarukoplayer_imgcenter(c,e,b,g),d=a[0],m=a[1],f=a[2],k=a[3]):"B"==a&&(b-c>g-e?(f=1.2*b,k=f/l):(k=1.2*g,f=k*l),d=(b-f)/2,m=(g-k)/2);nyarukoplayer_consolelog&&console.log(d+","+m+","+
f+","+k+","+l);return[d,m,f,k]}function nyarukoplayer_resizeendimg(){var a=$(window).width(),c=$(window).height();a=nyarukoplayer_imgcenter(nyarukoplayer_width[nyarukoplayer_now],nyarukoplayer_height[nyarukoplayer_now],a,c);x=a[0];y=a[1];w=a[2];h=a[3];$(".nyarukodiv").css({left:x,top:y,width:w,height:h})}
function nyarukoplayer_imgcenter(a,c,e,b){var g=0,d=0,m=a/c;a=e-a;var f=b-c,k=a/f;if(a>f){c=e;var l=c/m;d=(b-l)/2}else l=b,c=l*m,g=(e-c)/2;1<m?1.01<k&&k<m&&(a<f?(c=e,l=c/m,d=(b-l)/2,g=0):(l=b,c=l*m,g=(e-c)/2,d=0)):k>m&&1>k&&(a<f?(c=e,l=c/m,d=(b-l)/2,g=0):(l=b,c=l*m,g=(e-c)/2,d=0));return[g,d,c,l]}
function nyarukoplayer_audioinit(a){if("true"==$.cookie("disable"))nyarukoplayer_consolelog&&console.log("[NyarukoPlayer] Disabled by user."),$.isFunction(nyarukoplayerCallback_MusicDisabled)&&nyarukoplayerCallback_MusicDisabled();else{if(""!=nyarukoplayer_musicblock&&"true"!=$.cookie("nochkarea"))for(var c=navigator.language.substr(0,2),e=nyarukoplayer_musicblock.split("|"),b=0;b<e.length;b++)if(e[b]==c){nyarukoplayer_consolelog&&console.warn("[NyarukoPlayer] \u3053\u306e\u30da\u30fc\u30b8\u306f\u304a\u4f4f\u307e\u3044\u306e\u5730\u57df\u304b\u3089\u3054\u5229\u7528\u306b\u306a\u308c\u307e\u305b\u3093 Cannot use this page from the area to live");
$("#nyarukoplayer_audiodiv").remove();$.isFunction(nyarukoplayerCallback_MusicLanguageblock)&&nyarukoplayerCallback_MusicLanguageblock();return}$("#nyarukoplayer_musiccontrol").html('<source src="'+nyarukoplayer_musicfile+'" />');e=document.getElementById("nyarukoplayer_musiccontrol");c=$("#nyarukoplayer_audiodiv");e.load();e.pause();c.click(function(){document.getElementById("nyarukoplayer_musiccontrol").paused?nyarukoplayer_playmusic(!0):(nyarukoplayer_playmusic(!1),$("#nyarukoplayer_lrc").html(""))});
if(a){a=a.split("\n");for(var g=/\[\d{2}:\d{2}.\d{2}\]/g,d=[];!g.test(a[0]);)lineslines=a.slice(1);0===a[a.length-1].length&&a.pop();a.forEach(function(a,b,c){b=a.match(g);var f=a.replace(g,"");b.forEach(function(a,b,c){a=a.slice(1,-1).split(":");d.push([60*parseInt(a[0],10)+parseFloat(a[1]),f])})});a=0;c=d.length;for(var m;a<c;a++)for(b=0;b<c;b++)d[a][0]<d[b][0]&&(m=d[b],d[b]=d[a],d[a]=m);nyarukoplayer_lrc=[];for(b=0;b<d.length;b++)c=d[b][1],c=""==c?["",""]:c.split("|"),nyarukoplayer_lrc[b]=[d[b][0]+
nyarukoplayer_lrctime,c[0],c[1]];e.ontimeupdate=function(){for(var a=0,b=nyarukoplayer_lrc.length;a<b;a++){if(-1==nyarukoplayer_lrcoldtime)if(1>this.currentTime)nyarukoplayer_lrcoldtime=0;else break;var c=nyarukoplayer_lrc[a][0];if(this.currentTime>c&&nyarukoplayer_lrcoldtime<c){nyarukoplayer_lrcoldtime=this.currentTime;var d="";d=nyarukoplayer_cnlrc&&nyarukoplayer_lrc[a][2]?nyarukoplayer_lrc[a][2]:nyarukoplayer_lrc[a][1];"END"==d&&(d="",nyarukoplayer_consolelog&&console.log("[NyarukoPlayer] Lyric End."),
...
nyarukoplayer_lrctimeri=0,nyarukoplayer_lrctimers=""):(e.append(nyarukoplayer_lrctimers.charAt(nyarukoplayer_lrctimeri)),nyarukoplayer_lrctimeri++===nyarukoplayer_lrctimers.length&&(clearInterval(nyarukoplayer_lrctimer),nyarukoplayer_lrctimeri=0))},100);e.fadeTo(0,1,null)});nyarukoplayer_titlelrc&&(document.title="\u266a"+d)}break}}};$("#nyarukoplayer_lrc").click(function(){(nyarukoplayer_cnlrc=!nyarukoplayer_cnlrc)?$(this).html("(\u5207\u6362\u4e3a\u4e2d\u6587\u6b4c\u8bcd...)"):$(this).html("(\u5207\u6362\u4e3a\u539f\u7248\u6b4c\u8bcd...)")});
